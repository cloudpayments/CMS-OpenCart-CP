...
class ControllerPaymentCloudPayments extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension',
				'token=' . $this->session->data['token'] . '&type=payment', true)
		);

		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('payment/cloudpayments',
				'token=' . $this->session->data['token'],
				true)
		);

		$data['action'] = $this->url->link('payment/cloudpayments',
			'token=' . $this->session->data['token'], true);

		$data['cancel'] = $this->url->link('extension',
			'token=' . $this->session->data['token'] . '&type=payment', true);
...
//		$data['text_vat_none'] = $this->language->get('text_vat_none');
		...
		$url = rtrim(($this->request->server['HTTPS'] ? HTTPS_CATALOG : HTTP_CATALOG), '/') . '/index.php?route=';
		foreach (array('check', 'pay', 'fail', 'confirm', 'refund') as $type) {
			$data['notify_urls'][$type] = array(
				...
				'url'   => $url . 'payment/cloudpayments/notify-' . $type
			);
		}
        ...
        $this->response->setOutput($this->load->view('payment/cloudpayments', $data));
	}

	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/cloudpayments')) {
	...
	public function install() {
		...
		$this->model_extension_event->addEvent('cloudpayments', 'catalog/model/checkout/order/addOrderHistory/after', 'payment/cloudpayments/changeOrderStatus');
		$this->model_extension_event->addEvent('cloudpayments', 'catalog/model/checkout/order/deleteOrder/before', 'payment/cloudpayments/deleteOrderBefore');
		$this->load->model('payment/cloudpayments');
		$this->model_payment_cloudpayments->install();
	}
	...
	public function uninstall() {
		...
		$this->load->model('payment/cloudpayments');
		$this->model_payment_cloudpayments->uninstall();
	}
}