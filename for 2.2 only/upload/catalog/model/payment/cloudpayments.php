...
class ModelPaymentCloudPayments extends Model {
	...
	public function getMethod($address, $total) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('cloudpayments_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

		if ($this->config->get('cloudpayments_total') > 0 && $this->config->get('cloudpayments_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('cloudpayments_geo_zone_id')) {
			$status = true;
		...
		if ($status) {
			$method_data = array(
				...
				'terms'      => '',
				'sort_order' => $this->config->get('cloudpayments_sort_order')
			);
		}
